...
import {DriversQuery, DriversService, FeMessagesService} from "@bb/web/core";
...
export class DriversListComponent {
  ...
  drivers$ = this.driversService.filter.selectAllByFilters().pipe(
  ...
  loading$ = this.driversQuery.selectLoading();
  showEditModal = false;
  ...
  addDriver() {
    ...
    this.driversService.add(payload).subscribe(
      () => {
  ...
  delete(data: DriversInListDto) {
    this.modalService.confirm({
      ...
      nzOkDanger: true,
      nzOnOk: () =>
        this.driversService.delete(data.id).subscribe(
          () => {
            this.driversService.clearCache();
            this.driversService.get().subscribe();
  ...
  onSearchTermChange($event: any) {
    ...
    this.driversService.filter.setFilter({
      id: "name",
      value: searchTerm,
